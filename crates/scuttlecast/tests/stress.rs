...
async fn repeated_multi_slice_transfers() {
    ...
    for round in 0..5 {
        let received = common::transfer_to_file(40, 15000, &sent).await;
...
async fn multi_slice_transfer_to_three_receivers() {
    ...
    let receiving: Vec<_> = paths
        .iter()
        .map(|path| {
            let receiver = common::receiver(common::group(41), 15010);
            let path = path.clone();
            tokio::spawn(async move { receiver.recv_file(path).await.expect("receive") })
        })
        .collect();

    common::sender(common::group(41), 15010, 3)
...
async fn repeated_multi_megabyte_transfers() {
    ...
    for round in 0..3 {
        let received = common::transfer_to_file(42, 15020, &sent).await;
...
async fn multi_megabyte_transfer_to_three_receivers() {
    ...
    let receiving: Vec<_> = paths
        .iter()
        .map(|path| {
            let receiver = common::receiver(common::group(43), 15030);
            let path = path.clone();
            tokio::spawn(async move { receiver.recv_file(path).await.expect("receive") })
        })
        .collect();

    common::sender(common::group(43), 15030, 3)
        .send_stream(common::source(&sent))
        .await
        .expect("send");
